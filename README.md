...
Project này hiện thực bài toán ghép ảnh toàn cảnh (panorama stitching) bằng `Python + OpenCV`, với hai backend trích xuất đặc trưng là `ORB` và `SIFT`. Hệ thống hiện hỗ trợ hai mode hình học:
- `planar`: pipeline hiện tại cho `ORB` và `SIFT`
- `cylindrical`: mode `SIFT` kiểu notebook, dùng cylindrical projection và translation-only chaining
...
- ước lượng `homography` với fallback `affine partial` khi cần ở planar mode,
...
Chạy `SIFT + cylindrical`:

```bash
python3 -m src.main --feature sift --projection cylindrical --input_dir input --output_dir output --config configs/default.yaml
```
...
Khi chọn thêm `--projection cylindrical`, pipeline sẽ:
- chiếu từng ảnh lên mặt trụ trước khi matching,
- match trên ảnh cylindrical,
- ước lượng `homography` nhưng chỉ giữ `dx, dy`,
- chain transform từ ảnh giữa ra hai phía,
- blend tuần tự theo thứ tự center-out.
...
- `projection=cylindrical` hiện chỉ hỗ trợ `feature=sift`.
...
- Mode `cylindrical` được giữ tách biệt để không làm thay đổi hành vi của pipeline `planar` hiện có.
...
- Cylindrical mode hiện mới hỗ trợ `SIFT`, chưa hỗ trợ `ORB`.
